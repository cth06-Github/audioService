...
const AudioRecorder: React.FC<AudioProps> = (props): JSX.Element => {
  ...
  const startRecording = () => {
    ...
    mediaRecorder.current.ondataavailable = (event: any) => {
      ...
      setProcessedText(stringify(localAudioChunks));
    };
  ...
  const saveAudio = useCallback(() => {
    ...
    mediaRecorder.current.onstop = () => {
      ...
      setAudio(audioUrl);
      setAudioChunks([]);
      setProcessedText(processedText + dummyProcessedText); // simulate dummyTranscription as a value also returned by backend service
    };
  }, [audioChunks, props.downloadType, processedText]);
