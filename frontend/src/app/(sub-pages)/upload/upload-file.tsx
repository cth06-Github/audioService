...
import { dummyProcessedText } from "@/app/mock-data";
...
const UploadFile: React.FC<UploadProps> = (props): JSX.Element => {
  ...
  const [isProcessDone, setProcessDone] = useState<boolean>(false);
  ...
  const handleSendFile = async () => {
    setProcessDone(false); // reset transcribe done status before sending file
    ...
    setTimeout(() => {
      ...
      let valueFromBackend = dummyProcessedText; // receive the value return by the backend service
      setProcessedText(valueFromBackend);
      setProcessDone(true);
  ...
  const handlePressHome = () => {
    ...
    navCheck(!processedText && isFileSending, HOME, () =>
      router.push("/home")
    );
  };
  ...
  const handlePressLogout = () => {
    ...
    navCheck(!processedText && isFileSending, LOGOUT, logout);
  };
  ...
  return (
    <>
                ...
                <input
                  ...
                  onClick={(event) => {
                    if (
                      !(
                        !processedText ||
                        (processedText && isUploadDialogClicked.current)
                      )
                    )
      ...
      <TextBox
        ...
        withInfo={truncateName(processedFile, 25)}
        deleteFunc={handleDeleteText}
      />
      <SectionDialog
        actionItems={["Processing", "uploaded files"]}
